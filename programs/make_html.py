...
dir_path = lambda x : '' if x is '' else '../'*dir_count(x)
dir_count = lambda x : 0 if x is '' else len(x.split('/'))
slash = lambda x : '' if x is '' else '/'
...
for p in range(len(profile_input)):
    if(len(profile_input[p]) is 0):
        profile_arrange.append('')
    elif not(profile_input[p][0] is '#'):
...
FILE_NAME = 'NoneName_'+str(time.time()) if PROFILE['file_name'] is '' else PROFILE['file_name']
FILE_PATH = PROFILE['directory']+slash(PROFILE['directory'])+FILE_NAME+'.html'
OG_SITE_NAME = PROFILE['og:title'] if PROFILE['og:site_name'] is '' else PROFILE['og:site_name']
OG_TYPE = 'blog' if PROFILE['og:type'] is '' else PROFILE['og:type']
...
with open(MASTER_PATH+'/'+FILE_PATH, 'w') as f:
    f.write(head+middle+foot if PROFILE['insert preset'] is 't' else head+foot)
...
if(__name__ == '__main__') and (PROFILE['reset'] is 't'):
    reset()
